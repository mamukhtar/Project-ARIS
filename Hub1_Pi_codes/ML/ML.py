...
resting_data = pd.read_csv('/home/sensor-hub1/Documents/Pi_codes/ML/Training_set/sensor_data_resting.csv')
walking_data = pd.read_csv('/home/sensor-hub1/Documents/Pi_codes/ML/Training_set/sensor_data_walking.csv')
running_data = pd.read_csv('/home/sensor-hub1/Documents/Pi_codes/ML/Training_set/sensor_data_running.csv')
...
file_number = 4  # Set the initial file number to 0

while True:
    # Specify the original file path
    original_file_path = f"/home/sensor-hub1/Documents/Pi_codes/Hub1_data/Hub1_sensor_data_{file_number}.csv"
        
    # Specify the new folder path
    new_folder_path = "/home/sensor-hub1/Documents/Pi_codes/ML/ML_data"
    ...
    new_file_path = os.path.join(new_folder_path, f"Hub1_ML_data_{file_number}.csv")
    ...
    realtime_data = realtime_data.drop(['Time', 'Sensor1a', 'Amb_Temp', 'Sensor1b', 'Sensor1c', 'Sensor1d',
                                         'Sensor2a', 'Volume', 'Sensor2b', 'Volume_%', 'Sensor3a', 'Batttery_V', 
                                         'Sensor3b', 'Batttery_Level_%', 'Sensor4a', 'Pressure', 'Sensor4b', 'Pressure_%', 
                                         'Sensor5a', 'Body_Temp_C', 'Sensor5b', 'Sensor6a', 'BPM', 'IBI'], axis=1)
    ...
    sensor_thresholds = {
        'resting': {'Accel_X': [-0.1, -0.5, -0.8], 'Accel_Y': [0, 0.3, 0.5], 'Accel_Z': [9.93, 9.96, 10],'Heart_rate': [50, 65, 80], 'Body_Temp_F': [89, 90, 91]},
        'walking': {'Accel_X': [0.9, 1.4, 1.9], 'Accel_Y': [-0.5, 0, 0.5], 'Accel_Z': [9, 9.3, 10],'Heart_rate': [50, 70, 90], 'Body_Temp_F': [85, 86, 87]},
        'running': {'Accel_X': [1, 2, 3], 'Accel_Y': [-3, -1.5, 0], 'Accel_Z': [0.1, 0.3, 0.6],'Heart_rate': [130, 150, 170], 'Body_Temp_F': [75, 76, 77]},
    }
    ...
    body_temp_reading = body_temp_reading[
    ...
    if predicted_label == 'resting':
        ...
        if np.any(heart_rate_reading >= thresholds['Heart_rate'][0]) and np.any(body_temp_reading >= thresholds['Body_Temp_F'][0]) \
                and np.any(accel_x <= thresholds['Accel_X'][0]) and np.any(accel_y <= thresholds['Accel_Y'][0]) \
                and np.any(accel_z <= thresholds['Accel_Z'][0]):
            condition = 'normal'
        elif np.any(heart_rate_reading >= thresholds['Heart_rate'][1]) and np.any(body_temp_reading >= thresholds['Body_Temp_F'][1]) \
                and np.any(accel_x <= thresholds['Accel_X'][1]) and np.any(accel_y <= thresholds['Accel_Y'][1]) \
                and np.any(accel_z <= thresholds['Accel_Z'][1]):
            condition = 'normal -'
        elif np.any(heart_rate_reading >= thresholds['Heart_rate'][2]) and np.any(body_temp_reading >= thresholds['Body_Temp_F'][2]) \
                and np.any(accel_x <= thresholds['Accel_X'][2]) and np.any(accel_y <= thresholds['Accel_Y'][2]) \
                and np.any(accel_z <= thresholds['Accel_Z'][2]):
            condition = 'normal +'
        else:
            condition = 'abnormal'
    elif predicted_label == 'walking':
        thresholds = sensor_thresholds[predicted_label]
        if np.any(heart_rate_reading >= thresholds['Heart_rate'][0]) and np.any(
                body_temp_reading >= thresholds['Body_Temp_F'][0]) \
                and np.any(accel_x <= thresholds['Accel_X'][0]) and np.any(accel_y <= thresholds['Accel_Y'][0]) \
                and np.any(accel_z <= thresholds['Accel_Z'][0]):
            condition = 'normal'
        elif np.any(heart_rate_reading >= thresholds['Heart_rate'][1]) and np.any(
                body_temp_reading >= thresholds['Body_Temp_F'][1]) \
                and np.any(accel_x <= thresholds['Accel_X'][1]) and np.any(accel_y <= thresholds['Accel_Y'][1]) \
                and np.any(accel_z <= thresholds['Accel_Z'][1]):
            condition = 'normal -'
        elif np.any(heart_rate_reading >= thresholds['Heart_rate'][2]) and np.any(
                body_temp_reading >= thresholds['Body_Temp_F'][2]) \
                and np.any(accel_x <= thresholds['Accel_X'][2]) and np.any(accel_y <= thresholds['Accel_Y'][2]) \
                and np.any(accel_z <= thresholds['Accel_Z'][2]):
            condition = 'normal +'
        else:
        ...
        if np.any(heart_rate_reading >= thresholds['Heart_rate'][0]) and np.any(
            ...
            data.to_csv(f"/home/sensor-hub1/Documents/Pi_codes/ML/ML_data/Hub1_ML_data_{file_number}.csv", index=False) 
